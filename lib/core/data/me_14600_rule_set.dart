...
///
/// WHAT IS ENCODED HERE -- exactly three table rows, nothing else:
///
...
/// deductions 74 and 64 are printed constants on p. 24, not derived from
/// profile dimensions -- no Profile.width/depth value feeds any rule.
///
...
/// with vantauxCount = 2:
///
///   - dormant/montant placements are role-scoped one-piece positions
///     (top+bottom dormants at L, left+right at H; left+right montants),
///     so those rules yield fixed(1) per placement and the unit totals
///     emerge from the four/two placements themselves;
///   - a top-or-bottom traverse placement spans BOTH leaves' track half
///     ((L−64)/2 each), so that rule yields fixed(2) per placement --
///     top + bottom placements reach the documented 4 pieces.
///
...
/// montants centraux 14 619/620/630 (ProfileType.mullion), chicane 14 624.
